...
export async function signup(email: string, password: string, requestedRole: string | null, creatorRole: Role | null = null) {
  ...
  logger.info('signup called', { email, requestedRole, creatorRole });
  ...
  if (error) {
    ...
    logger.error('supabase.auth.signUp failed', error);
  ...
  if (dbError) {
    ...
    logger.error('Failed to insert user metadata', dbError);

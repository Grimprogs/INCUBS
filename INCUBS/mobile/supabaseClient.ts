...
try {
  // eslint-disable-next-line @typescript-eslint/no-var-requires
  localFallback = require('./supabase.env.js');
  console.log('localFallback loaded:', !!localFallback, localFallback);
} catch (_e) {
  // not present — that's fine; localFallback stays null
  console.log('localFallback failed to load:', _e);
}

// Hardcoded fallback for development (TEMPORARY - will use env vars in production)
const DEV_FALLBACK = {
  SUPABASE_URL: 'https://jawtvkgblqzprmxgssga.supabase.co',
  SUPABASE_ANON_KEY: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imphd3R2a2dibHF6cHJteGdzc2dhIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjY0MTA1MzUsImV4cCI6MjA4MTk4NjUzNX0.95sCQQmvC7Ng8823NfeLceBUBxEwkSy6WSF4amaHCN4'
};

let SUPABASE_URL = stripQuotes((process.env as any).SUPABASE_URL || getExpoExtra('SUPABASE_URL') || (localFallback && localFallback.SUPABASE_URL) || DEV_FALLBACK.SUPABASE_URL);
let SUPABASE_ANON_KEY = stripQuotes((process.env as any).SUPABASE_ANON_KEY || getExpoExtra('SUPABASE_ANON_KEY') || (localFallback && localFallback.SUPABASE_ANON_KEY) || DEV_FALLBACK.SUPABASE_ANON_KEY);

// Fallback: if still not found, try to load directly from supabase.env.js module
if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
  try {
    const envConfig = require('./supabase.env.js');
    SUPABASE_URL = SUPABASE_URL || envConfig.SUPABASE_URL;
    SUPABASE_ANON_KEY = SUPABASE_ANON_KEY || envConfig.SUPABASE_ANON_KEY;
  } catch (e) {
    // ignore
  }
}
...
if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
  ...
  try {
    ...
    console.log('supabase config sources', JSON.stringify({
      ...
      gotFromLocalFallback: !!(localFallback && (localFallback.SUPABASE_URL || localFallback.SUPABASE_ANON_KEY)),
      localFallbackKeys: localFallback ? Object.keys(localFallback) : []
    }));
    try {

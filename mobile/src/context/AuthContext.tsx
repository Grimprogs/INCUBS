...
const AuthContext = createContext<{
  ...
  signUp: (email: string, password: string, metadata?: { data: any }) => Promise<{ error: any | null; data?: any }>;
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      try {
        ...
        // Handle known Supabase auth errors such as invalid refresh token.
        const msg = err?.message || String(err);
        console.warn('supabase.getSession error:', msg);
        if (msg.includes('Invalid Refresh Token') || msg.includes('Refresh Token Not Found')) {
          // Clear any invalid session state server/client-side so app continues.
          try {
          ...
          // For other errors, rethrow so caller can observe (optional).
          session = null;
        }
      }
  ...
  // NOW WITH METADATA SUPPORT for recovery key hash
  async function signUp(email: string, password: string, metadata?: { data: any }) {
    try {
      // Build signup options with optional metadata
      const signUpOptions: any = { email, password };
      
      // Add metadata if provided (e.g., recovery_key_hash)
      if (metadata?.data) {
  ...
  async function setRole(r: Role) {
    ...
    const public_id = r ? `${r}-${Date.now().toString().slice(-6)}` : null;
